...
from etl.domain.services import ETLTransformAbstract
...
def test_transform_abstract(repository):
    # Arrange
    ...
    extract = ETLExtractSPY(repository)
    contact = extract.handle(contact)

    # Act
    transform = ETLTransformAbstract()
    res = transform.handle(contact=contact)
